...
<head>
    ...
    <title>Accueil</title>
    <meta name="author" content="Romain THOMAS & Kévin SOARES">
...
<body>
    ...
    <header>
        ...
        <nav class="transition-opacity">
            <button class="" onclick="actionpopup()">Filtres</button>
    ...
    <main class="transition-opacity">
        <div id="popup" class="popup" style="display: none;" >
            <button onclick="fermepopup()" class="close-button">
            ...
            <form action="index.php" method="POST">
                ...
                <input type="number" name="prix" placeholder="prix maximum">€
            
                <button type="submit" name="valid_filtres">Appliquer les filtres</button>
            </form>
        </div>
        <hr><br>
        
        <section id="top_accueil"></section>
    ...
    //affichage des photos des produits avec leur nom et marque
    $requete="SELECT Nom,Marque,Prix,Emplacement_img FROM Produits INNER JOIN Modeles ON Produits.ID_produit=Modeles.Ref_produit GROUP BY ID_produit ORDER BY ID_produit";
    $result=mysqli_query($conn,$requete);

        echo'<section id="vitrine_accueil">';
        
        while ($row=mysqli_fetch_assoc($result)){
            echo"<a href='pageIndiv.php?nom=".$row['Nom']."'><br>";
            
            //affichage produit et infos
            echo"<img src='".$row['Emplacement_img']."' alt='image modele'>";

            echo"<div class='details'>";
            echo"<p class='nom'>".$row['Nom']."</p>";
            echo"<p class='marque'>".$row['Marque']."</p>";
    ...
    //Code PHP pour gerer les filtres
    if(isset($_POST['valid_filtres'])){
        $marque=$_POST['marque'];
        ...
        $requete3="SELECT Nom,Emplacement_img FROM Produits INNER JOIN Modeles ON Produits.ID_produit=Modeles.Ref_produit WHERE Produits.Marque='".$marque."' OR Produits.Prix <='$prix' OR Modeles.Couleur='".$couleur."'";
        $result3=mysqli_query($conn,$requete);

        
        while ($row2=mysqli_fetch_assoc($result3)){
            echo'<a href="pageIndiv.php?nom='.$row2['Nom'].'"> <img src="'.$row2['Emplacement_img'].'" alt="image"> <p>'.$marque.' - '.$row2['Nom'].'</p></a>';
        }
